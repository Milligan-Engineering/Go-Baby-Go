...
/* NOTE! BTN_FORWARD, BTN_REVERSE, BTN_LEFT, and BTN_RIGHT have been changed, contrary to the drawings.
 * Typically, the port settings are as follows (which do not correspond with the drawings):
 * D6 - forward
 * D5 - reverse
 * D4 - right
 * D3 - left
 * 
 * The values for the port settings have been altered, and so have the code inside the functions do_forward(), do_reverse(), do_left(), and do_right().
 * These chages were made such that when the function is called, regardless of the logic of the code inside of the function, the vehicle would operate in BOTH the directions
 * indicated by the name of the function and the physical joystick direction.
 */
